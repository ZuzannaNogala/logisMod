...
Thanks to it we can bulid multiple models at once, just by putting theirs formulas after commas. As a result, we always get a list of models (even if there is only one).
...
=======
The user can build a model using `logisMod()` method.

```{r}
model <- logisMod(formula = nameBin ~ diameter + red + green + blue, data = citrus)
```
...
In our package there are few method to help the user choose a model. Firsty, method `countInfCrit()` computes values of criterias AIC and BIC for every possible set of predictors given by user.
...
deviance_test(model_H0 = model_test[[1]], model_H1 = model_test[[2]], alpha = 0.1)
```
...
drawROCsForEachModel(thres_hold_seq, "nameBin", 
                     createModels(citrus, nameBin ~ diameter + red + green + blue)[[1]])
```
...
We are creating models mostly beacuse we want to predict new values of dependent variable based on new observations. We provide function that can do this, it is fixed form of \code{stats::predict.glm}. For more details or different version see \link[stats]{predict.glm}.
